...
module.exports=function(code){
  ...
  var oldRoot=root;
  root=pairDefinitions(oldRoot);//Pair definitions
  var oldRoot=root;
...
*/
  ...
  function pairDefinitions(current){
    var out=new Tree(current.value);
    for(var i=0;i<current.children.length;i++){
      if(current.children[i].value==":="){//Definition
        var temp=out.children[out.children.length-1];
        current.children[i].add(temp);
        out.children.pop();
        out.add(current.children[i]);
        //out.children[out.children.length-1].children.reverse();
      }else{
        out.add(current.children[i]);
      }
    }
    ...
    for(var i=0;i<out.children.length;i++){
      out.children[i]=pairDefinitions(out.children[i]);
    }
